...
dataset = np.load("train_pack_299.npz")
...
inception = Models.inception_v3(pretrained=True,aux_logits=True,transform_input=True)
for param in inception.parameters():
	param.requires_grad = False
num_features = inception.fc.in_features
inception.fc = nn.Sequential(nn.Linear(num_features, num_classes),
								nn.Softmax(dim=1))
			...
			for inputs, labels in dataloaders[phase]:
				...
				with torch.set_grad_enabled(phase == 'train'):
					if phase == 'train':
						...
						# _, aux_preds = torch.max(aux_outputs,1)
				...
				# statistics
				# aux_loss += aux_loss.item() * inputs.size(0)
				running_loss += loss.item() * inputs.size(0)
				running_corrects += torch.sum(preds == labels.data)
				# aux_corrects += torch.sum(aux_preds == labels.data)

			epoch_loss = running_loss / dataset_sizes[phase]
			epoch_acc = running_corrects.double() / dataset_sizes[phase]
			# aux_epoch_loss = aux_loss / dataset_sizes[phase]
			# aux_epoch_acc = aux_corrects.double() / dataset_sizes[phase]
...
if False:
	...
	torch.save(model.state_dict(), 'InceptionV3_100epoch.ckpt')
	...
	np.savez("InceptionV3_100epoch.npz",valid_loss_plt=valid_loss_plt,valid_acc_plt=valid_acc_plt,loss_plt=loss_plt,acc_plt=acc_plt)
...
if True:
	...
	valid_loss_plt = Inception_100epoch['valid_loss_plt']
	valid_acc_plt = Inception_100epoch['valid_acc_plt']
	loss_plt = Inception_100epoch['loss_plt']
	acc_plt = Inception_100epoch['acc_plt']
	...
	model.load_state_dict(torch.load('InceptionV3_100epoch.ckpt'))
	model.eval()
	# print()
	test_label = []
	predicted_label = []
	...
	plot_confusion_matrix(cnf_matrix, classes=selected_breed_list, normalize=True,
							title='Inception_v3 confusion matrix')
